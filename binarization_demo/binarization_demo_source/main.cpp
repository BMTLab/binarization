...
int get_data(const char *name, uint_fast16_t *width, uint_fast16_t *height, unsigned char **raw)
{
	...
	const png_structp png = png_create_read_struct(PNG_LIBPNG_VER_STRING, NULL, NULL, NULL);
	if (!png)
		goto out;
	...
	*raw = (unsigned char *)malloc(*width * *height * sizeof(png_uint_16) + 1);

	if (!(raw && width > 0 && height > 0))
		goto out;

	/* Fill buffer */
	png_bytep *row_pointer = (png_bytep *)malloc(*width * sizeof(png_uint_16) + 1);

	for (uint_fast8_t pass = 0; pass < number_passes; pass++)
	{
		for (uint_fast16_t y = 0; y < *height; y++)
		{
			*row_pointer = (unsigned char *)*raw + y * *width;
...
int main(void)
{
	...
	if (!write_data(file_path_dst, width, height, raw, "output"))
	{
